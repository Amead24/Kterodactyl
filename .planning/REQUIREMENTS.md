...
- [ ] **INFRA-03**: Developer can run each test tier independently (make test, make test-integration, make test-e2e, make test-playwright)
- [ ] **INFRA-04**: Each test creates and cleans up its own resources without leaking state to other tests

### Go API Tests

- [ ] **GAPI-01**: Mod handler endpoints have httptest-based integration tests covering upload and list flows
- [ ] **GAPI-02**: Backup handler endpoints have httptest-based integration tests covering create, list, and restore flows
...
- [ ] **PW-01**: Playwright project is initialized with config, auth fixtures, and Chromium-only setup in a top-level `e2e/` directory
- [ ] **PW-02**: Auth fixture creates storageState for admin and regular user roles
